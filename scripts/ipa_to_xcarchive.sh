...
#
# Использование: ./scripts/ipa_to_xcarchive.sh [путь/к/MQGram.ipa]
...
if [ -z "$INPUT" ]; then
  ...
  for p in "bazel-bin/Telegram/MQGram.ipa" "bazel-bin/Telegram/MQGram"*.ipa "build/artifacts/TestFlight/MQGram.ipa" "$HOME/Downloads/MQGram.ipa"; do
    if [ -f "$p" ]; then
      INPUT="$p"
      break
    fi
...
if [ -z "$INPUT" ] || [ ! -f "$INPUT" ]; then
  echo "Использование: $0 <путь/к/MQGram.ipa>"
...
<dict>
	...
	<dict>
		...
		<string>com.GLEProject.MQGram</string>
