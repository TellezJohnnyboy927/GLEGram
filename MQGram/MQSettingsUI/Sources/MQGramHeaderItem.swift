// MARK: Swiftgram – MQGram settings header (icon + title + tagline)
import Foundation
import UIKit
...
public final class MQGramHeaderItem: ItemListControllerHeaderItem {
    ...
    public func isEqual(to: ItemListControllerHeaderItem) -> Bool {
        if let item = to as? MQGramHeaderItem {
    ...
    public func node(current: ItemListControllerHeaderItemNode?) -> ItemListControllerHeaderItemNode {
        if let current = current as? MQGramHeaderItemNode {
            current.item = self
            return current
        }
        return MQGramHeaderItemNode(item: self)
    }
}
...
final class MQGramHeaderItemNode: ItemListControllerHeaderItemNode {
