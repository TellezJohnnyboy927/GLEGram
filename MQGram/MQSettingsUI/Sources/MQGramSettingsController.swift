...
private func mqGramRootEntries(presentationData: PresentationData, accounts: [AccountInfo] = []) -> [MQGramEntry] {
...
private func filterGatedFeatures(entries: [MQGramEntry]) -> [MQGramEntry] {
    ...
    let dependentDisclosures: [MQGramDisclosureLink: SGBoolSetting] = [
    ...
    let dependentSliders: [MQGramSliderSetting: SGBoolSetting] = [
...
private func mqGramEntries(presentationData: PresentationData, contentSettingsConfiguration: ContentSettingsConfiguration?, state: MQGramSettingsControllerState, mediaBoxBasePath: String, accounts: [AccountInfo] = []) -> [MQGramEntry] {
    let lang = presentationData.strings.baseLanguageCode
...
public func mqGramSettingsController(context: AccountContext) -> ViewController {
    ...
    func buildMQGramTabController(tab: MQGramTab, args: SGItemListArguments<SGBoolSetting, MQGramSliderSetting, MQGramOneFromManySetting, MQGramDisclosureLink, AnyHashable>) -> ViewController {
        ...
        |> map { _, state, presentationData, contentSettingsConfiguration, accountsWithInfo -> (ItemListControllerState, (ItemListNodeState, SGItemListArguments<SGBoolSetting, MQGramSliderSetting, MQGramOneFromManySetting, MQGramDisclosureLink, AnyHashable>)) in
            let lang = presentationData.strings.baseLanguageCode
            ...
            let allEntries = mqGramEntries(presentationData: presentationData, contentSettingsConfiguration: contentSettingsConfiguration, state: tabState, mediaBoxBasePath: context.account.postbox.mediaBox.basePath, accounts: accounts)
    ...
    let signal: Signal<(ItemListControllerState, (ItemListNodeState, SGItemListArguments<SGBoolSetting, MQGramSliderSetting, MQGramOneFromManySetting, MQGramDisclosureLink, AnyHashable>)), NoError> = combineLatest(reloadPromise.get(), context.sharedContext.presentationData, context.sharedContext.activeAccountsWithInfo)
    |> map { _, presentationData, accountsWithInfo -> (ItemListControllerState, (ItemListNodeState, SGItemListArguments<SGBoolSetting, MQGramSliderSetting, MQGramOneFromManySetting, MQGramDisclosureLink, AnyHashable>)) in
        ...
        let entries = mqGramRootEntries(presentationData: presentationData, accounts: accounts)
        let listState = ItemListNodeState(
            presentationData: ItemListPresentationData(presentationData),
            entries: entries,
            style: .blocks,
