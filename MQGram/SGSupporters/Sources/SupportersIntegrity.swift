...
public enum SupportersIntegrity {
    ...
    /// Derive mqgramTab access from accumulated fragments.
    ///
    /// Expected XOR when all integrity checks pass AND mqgramTab == true:
    ...
    public static func computeAccessToken(
        ...
        let payload = "\(userId)|\(mqgramTab ? "1" : "0")|\(betaBuilds ? "1" : "0")"
    ...
    private static func deriveIntegrityKey(from masterKey: Data) -> Data {
        let salt = Data("mqgram-integrity-v1".utf8)
    ...
    /// - `mqgramTab` / `betaBuilds`:  raw access flags from the server response
    ///
