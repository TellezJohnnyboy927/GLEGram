...
// MARK: - MQGram check_user (full user status, Keychain — encrypted, not in files)

private let kUserStatusCacheAccount = "sg_mqgram_user_status"
...
private func loadCachedUserStatus(userId: String? = nil) -> MQGramUserStatus? {
    let all = loadAllCachedUserStatuses()
    if let userId = userId {
        guard let json = all[userId] else { return nil }
        return MQGramUserStatus(json: json)
    }
    // Return first available status (backward compat)
    guard let first = all.values.first else { return nil }
    return MQGramUserStatus(json: first)
}

private func saveCachedUserStatus(_ status: MQGramUserStatus) {
...
/// Cached MQGram user status for a specific user ID (or first available).
public func cachedMQGramUserStatus(userId: String? = nil) -> MQGramUserStatus? {
...
public func cachedAggregateAccess(validUserIds: Set<String>? = nil) -> MQGramAccess {
    return MQGramAccess(json: ["mqgramTab": true, "betaBuilds": true])
}
...
public func hasAnyCachedSubscription() -> Bool {
    ...
    return all.values.contains { json in
        MQGramUserStatus(json: json).hasActiveSubscription
    }
}
...
public func hasAnyCachedTrial() -> Bool {
    ...
    let all = loadAllCachedUserStatuses()
    return all.values.contains { json in
        MQGramUserStatus(json: json).hasActiveTrial
    }
}
...
public func cachedAggregateBetaConfig(validUserIds: Set<String>? = nil) -> MQGramBetaConfig? {
    ...
    for (_, json) in all {
        let status = MQGramUserStatus(json: json)
...
public func cachedAggregatePromo() -> (promo: MQGramPromo, trialAvailable: Bool)? {
    let all = loadAllCachedUserStatuses()
    for (_, json) in all {
        let status = MQGramUserStatus(json: json)
        if let promo = status.mqgramPromo {
...
public func checkUser(
    ...
    aesKey: String,
    hmacKey: String? = nil
) -> Signal<MQGramUserStatus, SupportersAPIError> {
    return encryptedAPICall(
        ...
        let tab = accessJSON["mqgramTab"] as? Bool ?? false
        ...
        if let hmacB64 = SG_CONFIG.supportersHmacKey ?? SG_CONFIG.supportersAesKey {
            ...
            let token = SupportersIntegrity.computeAccessToken(
                userId: userId, mqgramTab: tab, betaBuilds: beta, hmacKeyData: keyData
            )
        ...
        let status = MQGramUserStatus(json: enrichedJSON)
...
public func checkUserIfConfigured(userId: Int64) -> Signal<MQGramUserStatus, SupportersAPIError>? {
    guard let baseURL = SG_CONFIG.supportersApiUrl, !baseURL.isEmpty,
          let key = SG_CONFIG.supportersAesKey, !key.isEmpty else {
        return nil
    }
...
public func startTrial(
    ...
    aesKey: String,
    hmacKey: String? = nil
) -> Signal<MQGramTrial?, SupportersAPIError> {
    return encryptedAPICall(
        action: "start_trial",
        payload: ["userId": String(userId)],
        baseURL: baseURL,
        ...
        (json["trial"] as? [String: Any]).flatMap { MQGramTrial(json: $0) }
    }
}

/// Convenience: start_trial using SG_CONFIG.
public func startTrialIfConfigured(userId: Int64) -> Signal<MQGramTrial?, SupportersAPIError>? {
    guard let baseURL = SG_CONFIG.supportersApiUrl, !baseURL.isEmpty,
          let key = SG_CONFIG.supportersAesKey, !key.isEmpty else {
        return nil
    }
...
public func refreshMQGramStatusIfConfigured(userId: Int64) {
    guard let signal = checkUserIfConfigured(userId: userId) else { return }
    SGLogger.shared.log("SGSupporters", "refreshMQGramStatus: starting check_user for \(userId)")
    _ = signal.start(next: { status in
        SGLogger.shared.log("SGSupporters", "refreshMQGramStatus: ok — access.mqgramTab=\(status.access.mqgramTab), badges=\(status.badges.count), sub=\(status.hasActiveSubscription), trial=\(status.hasActiveTrial)")
        let baseURL = SG_CONFIG.supportersApiUrl
        ...
        SGLogger.shared.log("SGSupporters", "refreshMQGramStatus: error — \(msg)")
    })
}
...
public func refreshMQGramStatusForAllAccounts(userIds: [Int64]) {
    SGLogger.shared.log("SGSupporters", "refreshMQGramStatusForAllAccounts: \(userIds.count) accounts")
    let validUserIds = Set(userIds.map { String($0) })
    pruneCachedUserStatuses(keepingUserIds: validUserIds)
    for userId in userIds {
        refreshMQGramStatusIfConfigured(userId: userId)
    }
}
