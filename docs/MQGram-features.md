# MQGram: описание функций и полный исходный код
...
Точка входа в настройках MQGram (пункты меню, не вся функция `mqGramAppearanceEntries`):
...
Пункты в MQGram (фрагмент `MQGramSettingsController.swift`):
...
### Пункты в MQGram (`Swiftgram/SGSettingsUI/Sources/MQGramSettingsController.swift`)
...
/// MQGram / ghostgram-style: local voice morphing for outgoing voice messages (UserDefaults).
...
// MARK: - Entries builder (full list, no filter — like MQGram settings)
...
/// Filter by search query — same logic as filterSGItemListUIEntrires in MQGram settings: two-pass, keep search, keep sections that have matches.
...
*Файл: `MQGram-features.md` в корне репозитория.*
