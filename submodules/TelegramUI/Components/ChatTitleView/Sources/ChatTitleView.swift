...
public final class ChatTitleView: UIView, NavigationBarTitleView {
                        ...
                        if peerView.peerId.isReplies {
                            ...
                            if let peer = peerView.peer {
                                ...
                                if peer.id != self.context.account.peerId {
                                    ...
                                    if isVerifiedPeer {
                                        titleCredibilityIcon = isMQGramChannel ? .verifiedMQGram : .verified
                                    }
                            ...
                            if let isMuted {
                                ...
                                if let notificationSettings = peerView.notificationSettings {
                                    if case let .muted(until) = notificationSettings.muteState, until >= Int32(CFAbsoluteTimeGetCurrent() + NSTimeIntervalSince1970) {
                                        if titleCredibilityIcon != .verified && titleCredibilityIcon != .verifiedMQGram { // MARK: - MQGram
                                            titleRightIcon = .mute
                                        }
                                    }
                                }
    ...
    public func updateLayout(availableSize: CGSize, transition: ContainedViewLayoutTransition) -> CGSize {
        ...
        // MARK: - MQGram
        case .verifiedMQGram:
            titleCredibilityContent = .image(image: UIImage(bundleImageName: "MQGramVerifiedBadge"), tintColor: nil)
        case .developer:
        ...
        // MARK: - MQGram
        case .verifiedMQGram:
            titleVerifiedContent = .image(image: UIImage(bundleImageName: "MQGramVerifiedBadge"), tintColor: nil)
        case .developer:
            let developerColor = UIColor(rgb: 0x00A8FF)
        ...
        let titleCredibilitySize = self.titleCredibilityIconView.update(
            ...
            environment: {},
            containerSize: CGSize(width: self.titleCredibilityIcon == .verifiedMQGram ? 26.0 : 20.0, height: self.titleCredibilityIcon == .verifiedMQGram ? 26.0 : 20.0) // MARK: - MQGram
        )
        ...
        let titleVerifiedSize = self.titleVerifiedIconView.update(
            ...
            environment: {},
            containerSize: CGSize(width: self.titleVerifiedIcon == .verifiedMQGram ? 26.0 : 20.0, height: self.titleVerifiedIcon == .verifiedMQGram ? 26.0 : 20.0) // MARK: - MQGram
        )
