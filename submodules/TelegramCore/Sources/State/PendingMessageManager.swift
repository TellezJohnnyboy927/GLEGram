...
public final class PendingMessageManager {
    ...
    // MARK: - MQGram - Ghost delay disposables
    ...
    deinit {
        ...
        // MARK: - MQGram - Ghost delay cleanup
    ...
    func updatePendingMessageIds(_ messageIds: Set<MessageId>) {
        ...
        self.queue.async {
            ...
            for id in removedMessageIds {
                // MARK: - MQGram - Ghost delay cleanup on removal
    ...
    // MARK: - MQGram - Ghost delay: process overdue messages on app becoming active
        ...
        disposable.set(messages.start(next: { [weak self] messages in
            if let strongSelf = self {
                ...
                for (messageContext, message, type, contentUploadSignal) in messagesToUpload {
                    // MARK: - MQGram - Ghost-delayed send: show as "sent" in UI immediately, send to server after sendAt.
