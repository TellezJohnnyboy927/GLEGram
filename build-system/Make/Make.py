...
class BazelCommandLine:
    ...
    def get_startup_bazel_arguments(self):
        ...
        # MARK: MQGram — use system JDK to avoid SIGBUS crash with embedded JDK 24 on macOS 15.7.4+
        import shutil
    ...
    def invoke_build(self):
        ...
        if self.custom_target is not None:
            target_label = '//Telegram:' + self.custom_target if self.custom_target == 'MQGram' else self.custom_target
...
def build(bazel, arguments):
    ...
    if arguments.outputBuildArtifactsPath is not None:
        ...
        if os.path.exists(artifacts_path + '/MQGram.ipa'):
            os.remove(artifacts_path + '/MQGram.ipa')
        ...
        ipa_paths = glob.glob('{}/MQGram.ipa'.format(built_ipa_path_prefix)) or glob.glob('{}/MQGram.ipa'.format(built_ipa_path_prefix))
        if len(ipa_paths) == 0:
            print(f'Could not find the IPA at {built_ipa_path_prefix}/MQGram.ipa or {built_ipa_path_prefix}/MQGram.ipa')
            sys.exit(1)
        elif len(ipa_paths) > 1:
            print('Multiple matching IPA files found: {}'.format(ipa_paths))
            sys.exit(1)
        shutil.copyfile(ipa_paths[0], artifacts_path + '/MQGram.ipa')
