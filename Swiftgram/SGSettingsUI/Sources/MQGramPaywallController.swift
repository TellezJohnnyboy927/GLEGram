...
private struct MQGramBackgroundView: View {
    var body: some View {
        ZStack {
            LinearGradient(
...
@available(iOS 13.0, *)
private struct MQGramPaywallView: View {
    let promo: MQGramPromo
...
public func mqGramPaywallController(context: AccountContext, promo: MQGramPromo, trialAvailable: Bool) -> ViewController {
    if #available(iOS 13.0, *) {
        ...
        let swiftUIView = SGSwiftUIView<MQGramPaywallView>(
        ...
        return MQGramPaywallFallbackController(context: context, promo: promo, trialAvailable: trialAvailable)
    }
}

private final class MQGramPaywallFallbackController: ViewController {
    private let context: AccountContext
    private let promo: MQGramPromo
    private let trialAvailable: Bool

    init(context: AccountContext, promo: MQGramPromo, trialAvailable: Bool) {
    ...
    @objc private func subscribeTap() {
        // MARK: MQGram — redirect to support chat
