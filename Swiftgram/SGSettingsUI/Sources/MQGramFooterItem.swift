// MARK: Swiftgram – MQGram settings footer
import Foundation
import UIKit
...
public final class MQGramFooterItem: ItemListControllerFooterItem {
    ...
    public func isEqual(to: ItemListControllerFooterItem) -> Bool {
        if let item = to as? MQGramFooterItem {
    ...
    public func node(current: ItemListControllerFooterItemNode?) -> ItemListControllerFooterItemNode {
        if let current = current as? MQGramFooterItemNode {
            current.item = self
            return current
        }
        return MQGramFooterItemNode(item: self)
    }
}

final class MQGramFooterItemNode: ItemListControllerFooterItemNode {
    private let backgroundNode: ASDisplayNode
    private let titleNode: ImmediateTextNode
    private let linkNode: ImmediateTextNode
    private var validLayout: ContainerViewLayout?

    var item: MQGramFooterItem {
        didSet {
            updateItem()
